...
class Matakuliah:
    ...
    def simpan(self):
        self.conn = mydb()
        val = (self.__kodemk,self.__namamk,self.__sks,self.__prodi)
        sql="INSERT INTO matakuliah (kodemk,namamk,sks,prodi) VALUES " + str(val) 
    ...
    def update(self, id):
        self.conn = mydb()
        val = (self.__kodemk,self.__namamk,self.__sks,self.__prodi, id)
        sql="UPDATE matakuliah SET kodemk=%s, namamk=%s, sks=%s, prodi=%s WHERE idmk=%s"
    ...
    def updateByKODEMK(self, kodemk):
        self.conn = mydb()
        val = (self.__kodemk,self.__namamk,self.__sks,self.__prodi, kodemk)
        sql="UPDATE matakuliah SET kodemk=%s, namamk=%s, sks=%s, prodi=%s WHERE kodemk=%s"
        self.affected = self.conn.update(sql, val)
        self.conn.disconnect
        return self.affected

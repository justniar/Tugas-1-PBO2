...
class WindowDosen(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.txtNIDN.returnPressed.connect(self.search_data) # Jika menekan tombol Enter saat berada di textbox spesialis
    ...
    def select_data(self):
        try:
            ...
            self.gridDosen.setHorizontalHeaderLabels(['ID', 'NIDN', 'Nama Dosen', 'Email', 'Fakultas'])
    ...
    def search_data(self):
        try:           
            ...
            a = dsn.affected
            if(a!=0):
                self.txtNIDN.setText(dsn.nidn.strip())
                self.txtNama.setText(dsn.nama.strip())
                self.txtEmail.setText(dsn.email.strip())
                self.cboFakultas.setCurrentText(dsn.fakultas.strip())
    ...
    def save_data(self):
        try:
            ...
            fakultas=self.cboFakultas.currentText()
            if(self.edit_mode==False):   
                dsn.nidn = nidn
                dsn.nama = nama
                dsn.email = email
                dsn.fakultas = fakultas
                a = dsn.simpan()
